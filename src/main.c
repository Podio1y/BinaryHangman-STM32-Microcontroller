...
/*int main(void)
{
    ...
    for (int i = 0 ; i < 100 ; i++){
        ST7789_DrawCircle(180+i, 120, 60, LIGHTGREEN);
        ST7789_DrawCircle(179+i, 120, 60, BLACK);
        HAL_Delay(10);
    }
    HAL_Delay(2000);
...
//#ifdef BUTTON_BLINK
    ...
    while (count < 10)
    {
        while(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13)); //Waits for button press
        
        count++;
        HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
        //HAL_Delay(250);  // 250 milliseconds == 1/4 second
        sprintf(test, "Testing 123456789 - count: %i", count);
        SerialPuts(test);
        ST7789_DrawCircle(10, 10, 100, LIGHTGREEN);

        while(!HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13)); // waits for button release
    }*/
    //printf("%i", count);
    //ST7789_DrawRectangle(2, 2, 100, 100, LIGHTGREEN);
